...
parser = argparse.ArgumentParser(
    description=(
        "rafty. An implementation of " "the Raft algorithm for " "distributed consensus"
    )
)
parser.add_argument(
    "-c",
    "--config",
    dest="path_conf",
    help="Config file path. Default: rafty.persist/config",
)
parser.add_argument(
    "-s",
    "--storage",
    help=("Path for the persistent state" " directory. Default: rafty.persist"),
)
parser.add_argument(
    "-a", "--address", help=("This node address. Default: " "127.0.0.1")
)
parser.add_argument("-p", "--port", help="This node port. Default: 5254", type=int)
parser.add_argument(
    "--remote-address", action="append", default=[], help="Remote node address"
)
parser.add_argument(
    "--remote-port", action="append", default=[], type=int, help="Remote node port"
)
...
class Config:
    ...
    def _get(self):
        default = {
            ...
            "cluster": set(),
            "storage": "rafty.persist",
        }

        environ = {
            k[5:].lower(): v for (k, v) in os.environ.items() if k.startswith("rafty_")
        }
        ...
        path_conf = (
            cmdline["path_conf"] if cmdline["path_conf"] else config["path_conf"]
        )
        ...
        if cmdline["remote_address"] and cmdline["remote_port"]:
            config["cluster"].add(
                *zip(cmdline["remote_address"], cmdline["remote_port"])
            )
