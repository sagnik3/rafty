...
class State:
    ...
    def __init__(self, old_state=None, orchestrator=None):
        ...
        if old_state:
            ...
            self.persist = PersistentDict(
                join(config.storage, "state"), {"votedFor": None, "currentTerm": 0}
            )
            self.volatile = {
                "leaderId": None,
            ...
            msg.update(
                {
                    "leaderStatus": {
                        ...
                        "waiting_clients": {
                            k: len(v) for (k, v) in self.waiting_clients.items()
                        },
                    }
                }
            )
...
class Follower(State):
    ...
    def restart_election_timer(self):
        ...
        self.election_timer = loop.call_later(
            timeout, self.orchestrator.change_state, Candidate
        )
    ...
    def on_peer_request_vote(self, peer, msg):
        ...
        index_is_current = msg["lastLogTerm"] > self.log.term() or (
            ...
            and msg["lastLogIndex"] >= self.log.index
        )
        ...
        logger.debug(
            "Voting for %s. Term:%s Vote:%s Index:%s",
            peer,
            term_is_current,
            can_vote,
            index_is_current,
    ...
    def on_peer_append_entries(self, peer, msg):
        ...
        if "compact_data" in msg:
            self.log = LogManager(
                compact_count=msg["compact_count"],
            ...
            logger.warning(
                "Could not append entries. cause: %s",
                "wrong\
                term"
                if not term_is_current
                else "prev log term mismatch",
            )
...
class Leader(State):
    ...
    def send_append_entries(self):
        ...
        for peer in self.volatile["cluster"]:
            ...
            logger.debug(
                "Sending %s entries to %s. Start index %s",
                len(msg["entries"]),
                peer,
                self.nextIndex[peer],
            )
    ...
    def on_client_append(self, protocol, msg):
        ...
        self.on_peer_response_append(
            self.volatile["address"],
            {"success": True, "matchIndex": self.log.commitIndex},
        )
    ...
    def on_client_config(self, protocol, msg):
        ...
        before proceding."""
        pending_configs = tuple(
            filter(
                lambda x: x["data"]["key"] == "cluster",
        ...
        if pending_configs:
            timeout = randrange(1, 4) * 10 ** (0 if config.debug else -1)
            loop = asyncio.get_event_loop()
            self.config_timer = loop.call_later(
                timeout, self.on_client_config, protocol, msg
            )
            return
