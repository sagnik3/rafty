...
from rafty.server.main import setup as serverSetup
from rafty.client.distributedDict import DistributedDict
...
class Server:
    def __init__(self, args):
        ...
        self.config = {
            ...
            "storage": "rafty.{}.persist".format(args.port),
        }
    ...
    def repeat(self):
        ...
        shutil.rmtree("rafty.{}.persist".format(self.address[1]))
        print("Starting")
        self.create_server()
        ...
        for n, test_case in enumerate(test_cases):
            ...
            while not ok:
                ...
                if ok:
                    ...
                    with open(
                        "result_{}.json".format(datetime.datetime.now().isoformat()),
                        "w+",
                    ) as f:
    ...
    def send_test(self, case):
        ...
        while not ready:
            try:
                self.leader.refresh()
                client_count = len(
                    list(filter(lambda x: x.startswith("checkin"), self.leader))
                )
                print(
                    "Servers {}, Clients {}".format(
                        len(self.leader["cluster"]), client_count
                    )
                )
                ready = (
                    len(self.leader["cluster"]) == expected_servers
                    and client_count == expected_clients
                )
                if ready:
                    ready = input("Ready? ") == "y"
                else:
        ...
        case.update({"clients": client_count, "start_time": start_time.isoformat()})
...
test_cases = [
    {"entries": entries, "workers": workers, "dimention": dimention}
