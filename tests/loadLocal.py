...
parser.add_argument(
    "-d", type=int, default=100, help=("Log entry dimention" " in bytes")
)
...
def client_pool(func, entries_count, workers, additional_args=[]):
    ...
    return (statistics.stdev(finish_times), statistics.mean(finish_times) - start_time)
...
if __name__ == "__main__":
    ...
    print(
        "Finished pushing {} key/value pairs of size {} kB "
        "with {} clients".format(args.e, args.d / 1000, args.c)
    )
    print("Duration: {:.2f} sec, stdev: {:.2}".format(elapsed_time, stdev_time))
