...
from rafty.server.main import setup
...
class Pool:
    def __init__(self, server_ids):
        ...
        for config in self.configs.values():
            ...
            self.servers[config["test_id"]] = Process(
                target=self._run_server, args=(config,)
            )
    ...
    def stop(self, n):
        ...
        for x in n:
            ...
            if self.running[x]:
                ...
                self.servers[x] = Process(
                    target=self._run_server, args=(self.configs[x],)
                )
    ...
    def _run_server(self, config):
        setup(config)
        rafty_logger = logging.getLogger("rafty")
        rafty_logger.setLevel(logging.CRITICAL)
