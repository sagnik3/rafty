...
class BasicTest(unittest.TestCase):
    ...
    def test_0_diagnostics(self):
        ...
        expected = {
            ...
            "volatile": {
                ...
                "cluster": set(
                    (("127.0.0.1", 9112), ("127.0.0.1", 9110), ("127.0.0.1", 9111))
                ),
            },
            "log": {
                "commitIndex": -1,
                ...
                "compacted": {"count": 0, "term": None, "path": "STUB", "data": {}},
            },
        }
        ...
        diagnostics["volatile"]["cluster"] = set(
            map(tuple, diagnostics["volatile"]["cluster"])
        )
    ...
    def test_5_add_server(self):
        ...
        self.pool.servers[10] = Process(
            target=self.pool._run_server, args=(self.pool.configs[10],)
        )
        self.pool.start(10)
        sleep(1)
    ...
    def test_6_remove_server(self):
        ...
        self.assertEqual(
            set(map(tuple, d.diagnostic["volatile"]["cluster"])),
